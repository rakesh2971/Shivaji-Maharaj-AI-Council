...
const getClient = () => {
  // Access environment variables in Vite (Client-side)
  const env = (import.meta as any).env || {};
  
  // Prefer VITE_API_KEY for browser deployment, fallback to process.env for other contexts
  const apiKey = env.VITE_API_KEY || process.env.API_KEY;

  if (!apiKey) {
    console.error("Missing VITE_API_KEY. Please set this environment variable.");
  }
